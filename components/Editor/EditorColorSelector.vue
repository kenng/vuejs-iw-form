...
watch(() => props.colorList, () => {
    const lookup: Record<number, null> = {}

    const convertedCustomList = convertCustomColorList([...props.colorList])

    // Remove any subsequent duplicating colours
    const uniqueCustomList = convertedCustomList.filter(x => {
        if (!(x.rawValue in lookup)) {
            lookup[x.rawValue] = null
            return true
        } else {
            return false
        }
    ...
    if (props.replaceDefault) {
        ...
        const defaultListLookup = new Set(defaultColorList.map(x => x.rawValue))

        // Remove any colours that already exist in default colour list
        colorListInUse.value = defaultColorList.concat(
            uniqueCustomList.filter(x =>
                !defaultListLookup.has(x.rawValue)
            )
        )
    }
}, { immediate: true })
...
function insertColor(ev: Event) {
    ...
    const colorIndexInList = colorListInUse.value
        .findIndex(color => color.rawValue == newColor.rawValue
